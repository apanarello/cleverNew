/**
 * $RCSfile$ $Revision: 12540 $ $Date: 2011-06-23 14:55:10 -0500 (Thu, 23 Jun
 * 2011) $
 *
 ...
 * the License.
 */
package org.clever.Common.smack;
...
import org.clever.Common.smack.packet.XMPPError;
...
public class XMPPConnection extends Connection {
     ...
     * at
     * <tt>serviceName</tt> with the default port of 5222. Encrypted connections
     * (TLS) will be used if available, stream compression is disabled, and
     * standard SASL mechanisms will be used for authentication.<p>
     * <p/>
     ...
     * using the {@link #XMPPConnection(ConnectionConfiguration)}
     * constructor.<p>
     * <p/>
     ...
     *
     * @param serviceName the name of the XMPP server to connect to; e.g.
     * <tt>example.com</tt>.
     * @param callbackHandler the CallbackHandler used to prompt for the
    ...
    public void sendPacket(Packet packet, String cm) {
        if (!isConnected()) {
            throw new IllegalStateException("Not connected to server.");
        }
        if (packet == null) {
        ...
        packetWriter.sendPacket(packet, cm);
    }
     ...
     *
     * @param packetInterceptor the packet interceptor to notify of packets
     * about to be sent.
     ...
     * data so under high load the server performance might be affected.<p>
     * <p/>
     ...
     * compression negotiation has to be done before authentication took
     * place.<p>
     * <p/>
     ...
     * It basically creates and maintains a socket connection to the server.<p>
     * <p/>
     * Listeners will be preserved from a previous connection if the
     * reconnection occurs after an abrupt termination.
     *
     * @throws XMPPException if an error occurs while trying to establish the
     * connection. Two possible errors can occur which will be wrapped by an
     * XMPPException -- UnknownHostException (XMPP error code 504), and
     * IOException (XMPP error code 502). The error codes and wrapped exceptions
     ...
     *
     * @param wasAuthenticated true if the connection has already been
     * authenticated.
     */
