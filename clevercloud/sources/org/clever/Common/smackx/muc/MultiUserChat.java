/**
 * $RCSfile$ $Revision: 11613 $ $Date: 2010-02-09 05:55:56 -0600 (Tue, 09 Feb
 * 2010) $
 *
...
import org.clever.Common.smack.ConnectionCreationListener;
import org.clever.Common.smack.ConnectionListener;
import org.clever.Common.smack.MessageListener;
import org.clever.Common.smack.PacketCollector;
import org.clever.Common.smack.PacketInterceptor;
import org.clever.Common.smack.PacketListener;
import org.clever.Common.smack.SmackConfiguration;
import org.clever.Common.smack.Connection;
import org.clever.Common.smack.XMPPException;
import org.clever.Common.smack.filter.AndFilter;
import org.clever.Common.smack.filter.FromMatchesFilter;
import org.clever.Common.smack.filter.MessageTypeFilter;
import org.clever.Common.smack.filter.PacketExtensionFilter;
import org.clever.Common.smack.filter.PacketFilter;
import org.clever.Common.smack.filter.PacketIDFilter;
import org.clever.Common.smack.filter.PacketTypeFilter;
...
import org.clever.Common.smack.packet.Registration;
import org.clever.Common.smackx.Form;
import org.clever.Common.smackx.NodeInformationProvider;
import org.clever.Common.smackx.ServiceDiscoveryManager;
import org.clever.Common.smackx.packet.DiscoverInfo;
import org.clever.Common.smackx.packet.DiscoverItems;
import org.clever.Common.smackx.packet.MUCAdmin;
import org.clever.Common.smackx.packet.MUCInitialPresence;
import org.clever.Common.smackx.packet.MUCOwner;
...
public class MultiUserChat {
     ...
     * running. Make sure to provide a valid JID.
     */
     ...
     * @param user the user to check. A fully qualified xmpp ID, e.g.
     * jdoe@example.com.
     ...
     *
     * @param connection the XMPP connection to use for discovering information
     * about the room.
     * @param room the name of the room in the form "roomName@service" of which
     ...
     * @throws XMPPException if an error occured while trying to discover
     ...
     * services.
     *
     * @param connection the XMPP connection to use for discovering Multi-User
     * Chat services.
     * @return a collection with the XMPP addresses of the Multi-User Chat
     * services.
     * @throws XMPPException if an error occured while trying to discover MUC
     * services.
     */
     ...
     *
     * @param connection the XMPP connection to use for discovering hosted rooms
     * by the MUC service.
     * @param serviceName the service that is hosting the rooms to discover.
     * @return a collection of HostedRooms.
     * @throws XMPPException if an error occured while trying to discover the
     * information.
     */
     ...
     * as the room has been created.<p>
     *
     ...
     * should send an empty form after creating the room. {@link #sendConfigurationForm(Form)}
     * <p>
     *
     ...
     * (e.g. room already exists; user already joined to an existant room or 405
     * error if the user is not allowed to create the room)
     */
     ...
     *
     * @param nickname the nickname to use.
     * @throws XMPPException if an error occurs joining the room. In particular,
     * a 401 error can occur if no password was provided and one is required; or
     * a 403 error can occur if the user is banned; or a 404 error can occur if
     * the room does not exist or is locked; or a 407 error can occur if user is
     * not on the member list; or a 409 error can occur if someone is already in
     * the group chat with the same nickname.
     */
     ...
     * Joins the chat room using the specified nickname and password. If already
     * joined using another nickname, this method will first leave the room and
     * then re-join using the new nickname.<p>
     *
     * To control the amount of history to receive while joining a room you will
     * need to provide a configured DiscussionHistory object.<p>
     *
     * A password is required when joining password protected rooms. If the room
     * does not require a password there is no need to provide one.<p>
     *
     ...
     * a room.
     * @param timeout the amount of time to wait for a reply from the MUC
     * service(in milleseconds).
     * @throws XMPPException if an error occurs joining the room. In particular,
     * a 401 error can occur if no password was provided and one is required; or
     * a 403 error can occur if the user is banned; or a 404 error can occur if
     * the room does not exist or is locked; or a 407 error can occur if user is
     ...
     * @throws XMPPException if an error occurs asking the configuration form
     * for the room.
     */
     ...
     * @throws XMPPException if an error occurs setting the new rooms'
     * configuration.
     */
     ...
     * allow only room admins to add new members.<p>
     *
     ...
     * together with the instrucions or <tt>null</tt> if no registration is
     * possible.
     * @throws XMPPException if an error occurs asking the registration form for
     * the room or a 405 error if the user is not allowed to register with the
     * room.
     */
     ...
     * changing the user's affiliation from "none" to "member.<p>
     *
     ...
     * @throws XMPPException if an error occurs submitting the registration
     * form. In particular, a 409 error can occur if the desired room nickname
     * is already reserved for that room; or a 503 error can occur if the room
     ...
     * @throws XMPPException if an error occurs while trying to destroy the
     * room. An error can occur which will be wrapped by an XMPPException --
     ...
     * forward the invitation to the invitee.<p>
     *
     * If the room is password-protected, the invitee will receive a password to
     * use to join the room. If the room is members-only, the the invitee may be
     * added to the member list.
     ...
     *
     * @param presenceInterceptor the new packet interceptor that will intercept
     * presence packets.
     */
     ...
     * current room's subject will be received.<p>
     *
     * To be notified every time the room's subject change you should add a
     * listener to this room. {@link #addSubjectUpdatedListener(SubjectUpdatedListener)}
     * <p>
     *
     ...
     * @throws XMPPException if the new nickname is already in use by another
     * occupant.
     */
     ...
     *
     * @param nickname the nickname of the participant or visitor to kick from
     * the room (e.g. "john").
     * @param reason the reason why the participant or visitor is being kicked
     * from the room.
     * @throws XMPPException if an error occurs kicking the occupant. In
     * particular, a 405 error can occur if a moderator or a user with an
     * affiliation of "owner" or "admin" was intended to be kicked (i.e. Not
     * Allowed error); or a 403 error can occur if the occupant that intended to
     * kick another occupant does not have kicking privileges (i.e. Forbidden
     * error); or a 400 error can occur if the provided nickname is not present
     * in the room.
     */
     ...
     * room occupants.
     *
     * @param nicknames the nicknames of the visitors to grant voice in the room
     * (e.g. "john").
     ...
     * occupants.
     *
     * @param nicknames the nicknames of the participants to revoke voice (e.g.
     * "john").
     * @throws XMPPException if an error occurs revoking voice from a
     * participant. In particular, a 405 error can occur if a moderator or a
     * user with an affiliation of "owner" or "admin" was tried to revoke his
     * voice (i.e. Not Allowed error); or a 400 error can occur if the provided
     ...
     * occupants.
     *
     * @param nickname the nickname of the participant to revoke voice (e.g.
     * "john").
     * @throws XMPPException if an error occurs revoking voice from a
     * participant. In particular, a 405 error can occur if a moderator or a
     * user with an affiliation of "owner" or "admin" was tried to revoke his
     ...
     * 405 error can occur if a moderator or a user with an affiliation of
     * "owner" or "admin" was tried to be banned (i.e. Not Allowed error).
     */
     ...
     * the bare XMPP user ID of the user who initiated the ban.
     *
     * @param jid the bare XMPP user ID of the user to ban (e.g.
     * "user@host.org").
     ...
     * being on the member list).
     *
     * @param jid the XMPP user ID of the user to grant membership (e.g.
     * "user@host.org").
     * @throws XMPPException if an error occurs granting membership to a user.
     */
     ...
     * type members-only then the user will be removed from the room.
     *
     * @param jid the bare XMPP user ID of the user to revoke membership (e.g.
     * "user@host.org").
     * @throws XMPPException if an error occurs revoking membership to a user.
     */
     ...
     *
     * @param nicknames the nicknames of the occupants to grant moderator
     * privileges.
     ...
     *
     * @param nicknames the nicknames of the occupants to revoke moderator
     * privileges.
     ...
     *
     * @param jids the collection of bare XMPP user IDs of the users to grant
     * ownership.
     ...
     * defining room features as well as perform all administrative functions.
     *
     * @param jid the bare XMPP user ID of the user to grant ownership (e.g.
     * "user@host.org").
     * @throws XMPPException if an error occurs granting ownership privileges to
     * a user.
     */
     ...
     * ownership privileges to other users.
     *
     * @param jid the bare XMPP user ID of the user to revoke ownership (e.g.
     * "user@host.org").
     * @throws XMPPException if an error occurs revoking ownership privileges
     * from a user.
     */
     ...
     *
     * @param jids the bare XMPP user IDs of the users to grant administrator
     * privileges.
     * @throws XMPPException if an error occurs granting administrator
     ...
     *
     * @param jids the bare XMPP user IDs of the user to revoke administrator
     * privileges.
     * @throws XMPPException if an error occurs revoking administrator
     ...
     * administrator privileges from a member or unaffiliated user.
     *
     * @param jid the bare XMPP user ID of the user to revoke administrator
     * privileges (e.g. "user@host.org").
     * @throws XMPPException if an error occurs revoking administrator
     * privileges from a user.
     */
     ...
     * {@link org.clever.Common.smack.util.StringUtils#parseResource(String)}
     ...
     * user is not in the room.<p>
     *
     * @param user the room occupant to search for his presence. The format of
     ...
     * affiliation of the user in the room.<p>
     *
     * @param user the room occupant to search for his presence. The format of
     * user must be: roomName@service/nickname (e.g.
     * darkcave@macbeth.shakespeare.lit/thirdwitch).
     ...
     * not in the room).
     */
    public Occupant getOccupant(String user, Logger log) {
        Presence presence = occupantsMap.get(user);
        log.debug("Sono nel GETOCCUPANT " + presence.toString());
        if (presence != null) {
            log.debug("CREO OGGETTO OCCUPANTO CON IL VALORE " + presence.toString());

            return new Occupant(presence,log);
        }
        return null;
    }
     ...
     * packets sent to the group chat.
     *
     * @param listener a packet listener that was being notified of any presence
     * packets sent to the group chat.
     */
     ...
     * @throws XMPPException if an error occured while performing the request to
     * the server or you don't have enough privileges to get this information.
     */
     ...
     *
     * @param occupant occupant unique room JID (e.g.
     * 'darkcave@macbeth.shakespeare.lit/Paul').
     * @param listener the listener is a message listener that will handle
     * messages for the newly created chat.
     ...
     * @throws XMPPException if someone without appropriate privileges attempts
     * to change the room subject will throw an error with code 403 (i.e.
     * Forbidden)
     */
     ...
     * the new presence
     * @param newRole the new role of the user in the room after receiving the
     * new presence
     * @param isUserModification whether the received presence is about your
     ...
     * before receiving the new presence
     * @param newAffiliation the new affiliation of the user in the room after
    ...
    private static class InvitationsMonitor implements ConnectionListener {
         ...
         *
         * @param connection the connection to monitor for possible room
         * invitations
         */
         ...
         * fired anytime an invitation is received.<p>
         *
